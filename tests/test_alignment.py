...
def synthetic_sheet(shape=(64, 64, 64), sheet_z=32.0, sigma=1.6, amplitude=140.0,
                    background=40.0, noise=0.0, seed=0):
...
def test_perfectly_placed_label_reports_zero_offset():
    ...
    result = ridge_alignment(volume, planar_label(shape, 32), n_samples=4000,
...
def test_a_planted_displacement_is_recovered(displacement):
    ...
    shape = (64, 64, 64)
    volume = synthetic_sheet(shape, sheet_z=32.0)
    result = ridge_alignment(volume, planar_label(shape, 32 + displacement),
                              n_samples=4000, orient_field=upward(shape))
...
def test_noise_raises_offset_spread_but_not_the_bias():
    shape = (64, 64, 64)
    clean = ridge_alignment(synthetic_sheet(shape, 32.0), planar_label(shape, 32),
                            n_samples=4000, orient_field=upward(shape))
    noisy = ridge_alignment(synthetic_sheet(shape, 32.0, noise=12.0), planar_label(shape, 32),
                            n_samples=4000, orient_field=upward(shape))
