...
dirs = sorted(d for d in glob.glob("/workspace/corpus_meshes/*/")
...
for d in dirs:
    ...
    rs = np.array([p["range"] for p in profs]) if profs and isinstance(profs[0], dict) \
        else np.array([np.ptp(p) for p in profs])
    pk = np.array([abs(np.argmax(p) - len(p) // 2) for p in profs]) if not isinstance(profs[0], dict) \
    ...
    print(f"{name[:44]:44s} {rs.size:6d} {np.median(rs):8.1f} "
