...
for f in sorted(glob.glob("/workspace/onsheet_corpus/*.json")):
    ...
    if not m:
        continue
    rows.append({"name": n, "day": m.group(1),
...
term, interior = [], []
print(f"{'series':10s} {'n':>3s} {'windings':>12s} {'last w':>7s} {'last frac':>10s} {'interior median':>16s}")
for day, rs in sorted(series.items()):
    ...
    print(f"{day:10s} {len(rs):3d} {f'{rs[0][chr(119)]:.0f}-{rs[-1][chr(119)]:.0f}':>12s} "
...
print(f"interior patches  n={len(interior):3d}  median {np.median(interior):.2f}  min {min(interior):.2f}")
