...
def load(pattern, scroll_from_name):
    ...
    for f in glob.glob(pattern):
        ...
        if w is None or not m:
            continue
        out.append({"scroll": scroll, "day": m.group(1), "w": w,
                    "range": t["range_median"], "name": n})
    return out
...
print(f"{'scroll':13s} {'series':10s} {'n':>3s} {'last w':>7s} {'last range':>11s} "
...
if hits == len(prim) and prim:
    ...
    print("not all terminal patches are worst -> reported as pre-registered, "
